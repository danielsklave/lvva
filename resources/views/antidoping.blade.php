...
@section('body')
    ...
    <h1 class="text-xl font-bold">Biežāk uzdotie jautājumi</h1>
    @php
        $arr = [
            [
    ...
    <div id="accordion-open" data-accordion="open">
        @foreach ($arr as $item)
            <h2 id="accordion-open-heading-{{ $loop->index }}">
                <button type="button" class="flex items-center justify-between w-full p-5 font-medium text-left text-gray-500 border border-b-0 border-gray-200 focus:ring-4 focus:ring-gray-200 " data-accordion-target="#accordion-open-body-{{ $loop->index }}" aria-expanded="false" aria-controls="accordion-open-body-{{ $loop->index }}">
            ...
            <div id="accordion-open-body-{{ $loop->index }}" class="hidden" aria-labelledby="accordion-open-heading-{{ $loop->index }}">
